...
@Injectable()
export class CriticalSectionsAuthorizer implements CustomAuthorizer<any> {
  ...
  async authorize(
    ...
    if (
      ...
      !operator.role.permissions.includes(OperatorPermission.Critical_View)
    ) {
      throw new UnauthorizedException();
    }
    if (
      !authorizerContext.readonly &&
      !operator.role.permissions.includes(OperatorPermission.Critical_Edit)
    ) {
      if (
